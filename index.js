...
const main = async () => {
    ...
    await Shell.runSilently(`mv SafariExts/*.appex ./Payload/Discord.app/PlugIns/ & wait $!`, (stderr) => {
        Shell.write(stderr
            ? `${S.FAILURE} An error occurred while copying Safari Extensions to ${M.PINK}\"${IPA_DIR}\".${M.ENDC}\n`
            : `${S.SUCCESS} Successfully copied Safari Extensions ${M.PINK}\"${IPA_DIR}\"${M.GREEN} into ${M.PINK}\"./Payload/Discord.app/PlugIns\".${M.ENDC}\n`);
    });
    await D.logDivider();
    ...
    await Shell.runSilently(`plutil -replace CFBundleDisplayName -string "CoolCord" ${MAIN_PLIST} & wait $!`, (stderr) => {
        Shell.write(stderr
    ...
    if (true) { // I don't care. bundling this in every releases.
        ...
        await Shell.run(`plutil -replace NSFaceIDUsageDescription -string "Face ID and Touch ID authentication on CoolCord with supported plugins" ${MAIN_PLIST} & wait $!`, (stderr) => {
            Shell.write(stderr
